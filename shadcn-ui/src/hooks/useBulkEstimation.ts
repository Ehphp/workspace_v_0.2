...
interface UseBulkEstimationReturn {
    ...
    start: (requirements: BulkRequirement[], listTechPresetId: string | null) => Promise<void>;
...
export function useBulkEstimation(): UseBulkEstimationReturn {
    ...
    const start = useCallback(async (
        ...
        const estimable = requirements.filter(r => {
            const techId = r.technology_id || r.tech_preset_id || listTechPresetId;
        ...
        for (let i = 0; i < estimable.length; i += MAX_CONCURRENT) {
            ...
            const promises = batch.map(async (req) => {
                ...
                try {
                    const technologyId = req.technology_id || req.tech_preset_id || listTechPresetId;
