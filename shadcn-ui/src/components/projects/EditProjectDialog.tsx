...
export function EditProjectDialog({ open, onOpenChange, project, onSuccess }: EditProjectDialogProps) {
    ...
    useEffect(() => {
        if (open && project) {
            ...
            setTechnologyId(project.technology_id || project.tech_preset_id || '__NONE__');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = await updateProject(project.id, {
            ...
            description,
            owner,
            technologyId: technologyId === '__NONE__' ? null : technologyId,
            status,
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[1280px] w-[95vw]">
                <DialogHeader>
