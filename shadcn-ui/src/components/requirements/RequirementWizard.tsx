...
import { WizardStepTechnicalAnalysis } from './wizard/WizardStepTechnicalAnalysis';
...
export function RequirementWizard({ projectId, projectContext, onSuccess, onCancel, isOpen }: RequirementWizardProps) {
    ...
    useEffect(() => {
        ...
        if (techId && !data.technologyId) {
            fetchTechnology(techId).then((tech) => {
                if (tech) {
                    updateData({ technologyId: tech.id, techCategory: tech.code || '' });
                }
    ...
    const steps = [
        ...
        { title: 'Technical Analysis', component: WizardStepTechnicalAnalysis },
    ...
    const handleSave = async (_estimationResult: EstimationResult) => {
        ...
        try {
            // Title: prefer interview-generated title, fallback to default
            const finalTitle = data.title || 'Untitled requirement';
            ...
            const requirement = await createRequirement({
                ...
                technology_id: data.technologyId || null,
            });
