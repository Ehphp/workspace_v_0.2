...
interface Particle {
    ...
    sizeFactor: number;
    hueVariation: number;
    phaseInfluence: number;  // For varying mouse influence
    seed: number;
}
...
export const RingParticlesCanvas: React.FC<RingParticlesCanvasProps> = ({
    ...
    const initializeParticles = useCallback(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            particlesRef.current.push({
                angleBase: (2 * Math.PI * i) / count,
                jitterAngle: (seededRandom(particleSeed) - 0.5) * 0.3,
                jitterRadius: (seededRandom(particleSeed + 1) - 0.5),
    ...
    }, [config.seed, config.particleCount, config.responsive.maxParticlesMobile]);
    ...
    const renderFrame = useCallback(() => {
        ...
        for (const particle of particlesRef.current) {
            ...
            if (mouseEnabled) {
                ...
                const dy = mousePy - y;
                const dist = Math.sqrt(dx * dx + dy * dy);
                const maxDist = Math.min(width, height) * 0.2; // 20% dello schermo - effetto più localizzato
