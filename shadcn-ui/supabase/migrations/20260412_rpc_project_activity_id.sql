-- ============================================
-- UPDATE save_estimation_atomic RPC
-- Support project_activity_id in estimation_activities
-- ============================================
-- The check constraint "chk_activity_or_project_activity" requires that
-- at least one of activity_id or project_activity_id is NOT NULL.
-- This migration updates the function body so it reads project_activity_id
-- from the p_activities JSONB and writes it to the estimation_activities row.
--
-- No signature change — CREATE OR REPLACE replaces the body in-place.
...
BEGIN
    ...
    -- Supports both global catalog (activity_id) and project-scoped (project_activity_id).
    -- When project_activity_id is present, activity_id is forced to NULL.
    -- The check constraint chk_activity_or_project_activity ensures at least one is set.
    ...
    SELECT
        v_estimation_id,
        -- Use activity_id only when project_activity_id is absent
        CASE
            WHEN NULLIF(item->>'project_activity_id', '') IS NOT NULL THEN NULL
            ELSE NULLIF(item->>'activity_id', '')::UUID
        END,
        NULLIF(item->>'project_activity_id', '')::UUID,
...
GRANT EXECUTE ON FUNCTION save_estimation_atomic(
    ...
    JSONB, JSONB, JSONB, TEXT, JSONB, UUID, UUID, UUID
) TO authenticated;
...
Supports both global catalog activities (activity_id) and
project-scoped activities (project_activity_id).
All operations are transactional — either all succeed or none.';
